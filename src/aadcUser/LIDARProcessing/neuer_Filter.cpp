#include "neuer_Filter.h"
#include <math.h>

ADTF_PLUGIN("neuer_Filter",cLIDARProcessing);
...
//print lines & Clustering// 1-0-1 with random amount of zeros=object
for(int i=0;i<counter-2;i++){
   ...
   if( (breakpoints[i] == 0 && breakpoints[i+1] == 1) && i!= Grad90Measurement){
        line(inputImage, Point(xCoord_img[i],yCoord_img[i]), Point(xCoord_img[i+1],yCoord_img[i+1]), Scalar(0,0,255), 2, 8, 0);
        iend=i+1;
        count_Obj++;
        ...
        Lidar_Output_[count_Obj]=Objekt;
        min_Radius=12001;
   }
   
}

    circle(inputImage, Point(320,480), 5, Scalar(255,0,0), 3, 8, 0); //Ego position
   
    //--------------------------------------------------------------------------------------------------  
    //returning the object data of Lidar Input in sensor Coordinates
    //auto Lidar_Output = output_sample_data<tLaserScannerData>(get_sample_time(pSample));   
